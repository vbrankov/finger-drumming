...
fixed 8 steps in from the left, 40 columns are visible. The strip of cells is
built once per run for the whole pass (plus a count-in bar before and a
wrap bar after) with plain DOM, never rebuilt while playing — replacing the
content under a composited layer shows a stale raster for a frame — and
cells are patched in place as results arrive. Every frame only its CSS
transform is set directly on the element from the audio clock, outside
React; the fractional position lives in a ref, and React state holds a
coarse position (per bar for songs, per step for a pattern's static grid,
per beat during the count-in). The content bar lines carry the pattern name
