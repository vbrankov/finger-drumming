...
can't be deleted while a song uses it. When the list grows, **style chips**
above it filter by style; a pattern tagged *fill* is a one-off bar meant to
end a phrase.

## Library

The **Library** tab holds about a thousand more grooves and fills you can
add to your patterns one at a time or by the filter ("Add all shown"). They
come from the [Groove MIDI Dataset](https://magenta.tensorflow.org/datasets/groove)
(Google Magenta, CC BY 4.0): ten drummers playing to a click on an
electronic kit, in eighteen styles from rock and funk to jazz, Afro-Cuban
and New Orleans. Each performance was snapped to the 16th grid; a groove is
the bar the drummer kept coming back to (two bars where they alternated),
a fill is the bar they played. Ghost notes and accents are the drummer's
own dynamics, relative to how hard they hit that drum; swing is detected
from where their off-beats land. **Practice** adds the pattern and opens it;
what you add is yours to edit, share and build songs from. Filter by style,
grooves vs fills, difficulty, or search by name.

The library is generated by `scripts/gmd-patterns.py`; other collections
